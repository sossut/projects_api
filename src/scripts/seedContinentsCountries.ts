...
const countries = [
  ...
  { name: 'Eswatini (fmr. "Swaziland")', code: 'SZ', continent: 'Africa' },
...
async function seedContinents() {
  for (const continent of continents) {
    ...
    if (rows.length > 0) {
      console.log(`Continent already exists: ${continent.name}`);
      continue;
    }
    ...
    console.log(`Inserted continent: ${continent.name}`);
  }
  console.log('Continents seeding complete');
}

async function seedCountries() {
  for (const country of countries) {
    // Get continent id
    const [contRows] = await promisePool.query<any[]>(
      'SELECT id FROM continents WHERE name = ?',
      [country.continent]
    );
    if (contRows.length === 0) {
      console.log(`Continent not found for country: ${country.name}`);
      continue;
    }
    const continentId = contRows[0].id;
    ...
    await promisePool.execute(
      ...
      [country.name, country.code, continentId]
    );
