...
const getProjectsByCity = async (
  ...
  if (rows.length === 0) {
    throw new CustomError(`No projects found in city: ${city}`, 404);
  }
  return parseProjectRows(rows);
};
...
const getProjectsByStatus = async (
  ...
  if (rows.length === 0) {
    throw new CustomError(`No projects found with status: ${status}`, 404);
  }
  return parseProjectRows(rows);
};

const getProjectsByMetroArea = async (
  metroArea: string,
  sortBy: string = 'id',
  order: 'ASC' | 'DESC' = 'ASC'
): Promise<Project[]> => {
  ...
  const [rows] = await promisePool.query<GetProject[]>(
    ...
    LEFT JOIN project_medias ON projects.id = project_medias.project_id
    LEFT JOIN source_links ON projects.id = source_links.project_id
    WHERE metro_areas.name = ?
    GROUP BY projects.id
    ORDER BY ${validSortBy} ${validOrder}`,
    [metroArea]
  );
  if (rows.length === 0) {
    throw new CustomError(`No projects found in metro area: ${metroArea}`, 404);
  }
  return parseProjectRows(rows);
};
